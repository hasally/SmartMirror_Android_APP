...
public class CustomMemberInsertTask extends AsyncTask<Void, Void, String> {
    ...
    public CustomMemberInsertTask(Context context) {
        mContext = context;
    ...
    protected String doInBackground(Void... strings) {
        try {
            ...
            //192.168.0.15
            URL url = new URL(ip_name+"register.do");
            ...
            System.out.println("서버로 보낼 회원 정보 데이터-------------------------");

            jobj = new JSONObject();
            jobj.put("mirror_id", userInfo.getKeyMirrorId());
            jobj.put("id",userInfo.getKeyId());
            jobj.put("pw",userInfo.getKeyPw());
            jobj.put("name",userInfo.getKeyName());
            jobj.put("phone",userInfo.getKeyPhone());
            jobj.put("weather_loc",userInfo.getKeyWeatherLoc());
            jobj.put("subway_loc",userInfo.getKeySubwayLoc());

            System.out.println("서버로 보낼 회원 정보 확인: "+ jobj);
